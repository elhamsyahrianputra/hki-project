...
class Brand extends Model
{
    ...
    public static function countStatus(String $user_id = null) {
        ...
        foreach ($brands as $brand) {
            $statuses[] = $brand->lastStatus();
        }

        return [
            'total' => collect($statuses)->count(),
            'accepted' => collect($statuses)->where('status', 'accepted')->count(),
            'process' => collect($statuses)->where('status', 'applied')->count() + 
                         collect($statuses)->where('status', 'revision')->count() + 
                         collect($statuses)->where('status', 'revised')->count(),
            'rejected' => collect($statuses)->where('status', 'rejected')->count(),
        ];
    }
    ...
    public function statuses() {
        return $this->hasMany(BrandStatus::class)->latest();
    }
}
